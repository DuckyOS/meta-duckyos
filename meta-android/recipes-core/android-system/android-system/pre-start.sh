...
if [ -e $sys_vendor ] && ! mountpoint -q -- /android/vendor; then
...
cat ${fstab} | while read line; do
    ...
    if [ -n "${LXC_ROOTFS_PATH}" ] && [[ ${2} != /mnt/* ]]; then
        # /mnt is recursively binded via the LXC configuration
        mount -o bind ${2} "${LXC_ROOTFS_PATH}/${2}"
    fi
done
