...
panic() {
    ...
    if [ -d $ANDROID_USB ]; then
        usb_setup_android_usb "Halium"
    else
        usb_setup_configfs "Halium"
    fi
...
mount_kernel_modules() {
    # Avoid overriding kernel modules in DuckyOS
...
process_bind_mounts() {
    ...
    mkdir -p $datadir/duckyos-lxc-android
    mount -o bind ${rootmnt}/var/lib/lxc/android $datadir/duckyos-lxc-android
    # this is also needed, in the scenario of a system-as-root mount
    mount --move ${rootmnt}/var/lib/lxc/android/rootfs $datadir/duckyos-lxc-android/rootfs || true
    ...
    if [ ! -e $datadir/var/lib/lxc/android ]; then
        ...
        ln -sf ../../../userdata/duckyos-data/duckyos-lxc-android $datadir/var/lib/lxc/android
    fi
